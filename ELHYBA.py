...
print("البوت يعمل بنجاح !\تم التطوير بواسطة تايسون : @T_1_Y_1")
...
async def robot(event):
    ...
    try:
        if id not in steps:
            steps[id] = 1
            return await event.reply("**مرحبًا بك في المصدر الذكي للحذف ، وحرق حساب Telegram\n\n يمكنك رقمك من خلال الزر التالي بالاسفل🖤⚡️•**", buttons = [[Button.request_phone("• جهاتي •", resize = True)]])
        elif "start" in text or text == "• تحليل •":
            steps[id] = 1
            await event.reply("**مرحبًا بك في حذف حسابات Telegram\n\n يمكنك رقمك من خلال الزر بالاسفل🖤⚡️•**", buttons = [[Button.request_phone("• جهاتي •", resize = True)]])
            delete.remove(id)
            return
        step = steps[id]
        if step  == 1:
            if event.contact:
                ...
                if not res:
                    steps[id] = 2
                    return await event.reply("**✧¦ بنجاح ، تم إرسال الرمز اليك بنجاح ، يرجى القيام توجيه الرسالة يحي على الرمز.♥️•**", buttons = [[Button.text("• تحليل •", resize = True)]])
                elif res == 1:
                    return await event.reply("**✧¦ الاك ده خول مش عوز يتحذف **")
                elif res == 2:
                     return await event.reply("**✧¦  يسطا حاول ف وقت تاني و متصدعنيش ♥•**")
                else:
                    return await event.reply("**✧¦ يسطا حاول ف وقت تاني و متصدعنيش ♥•**")
            else:
        ...
        if step == 2:
            if event.forward:
                ...
                if not res:
                    del steps[id]
                    msg = await event.reply("**برا يكسمك ♥•**")
                    #sleep(1);input('wait ')
                    delete.delete(id)
                    delete.remove(id)
                elif res == 1:
                    return await event.reply("**✧¦ الاك ده خول مش عوز يتحذف **")
                     ...
                     return await event.reply("**✧¦ يسطا حاول ف وقت تاني و متصدعنيش ♥ ️ •**")
                elif res == 3:
                     return await event.reply("**✧¦ الكود غير صحيح أو منتهي الصلاحية ♥•**")
                else:
                    return await event.reply("**✧¦  يسطا حاول ف وقت تاني و متصدعنيش ♥ ️ •**")
            else:
                return await event.reply("**✧¦ يرجى فقط القيام باعدة توجيه الرسالة التي وصلتك ♥•**")
    except Exception as e:
        print(type(e),e)
